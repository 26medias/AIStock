...
// Get the Burg Coeffs (regression)
timeseries.prototype.burgCoeff = function(options) {
	// http://emptyloop.com/technotes/A%20tutorial%20on%20Burg%27s%20method,%20algorithm%20and%20recursion.pdf
	...
	var N = options.data.length-1;
	var m = options.coeffs.length;
	...
	//vector<double> f( x );
	//vector<double> b( x );
	var f = _.map(options.data, function(point) {
		return point[1];
	});
	var b = _.map(options.data, function(point) {
		return point[1];
	});
	...
	for (k = 0; k < m; k++ ){
		...
		// UPDATE Ak
		for (n = 0; n <= ( k + 1 ) / 2; n++ ){
			var t1 = Ak[n] + mu * Ak[k + 1 - n];
			var t2 = Ak[k + 1 - n] + mu * Ak[n];
			Ak[n] = t1;
			Ak[k + 1 - n] = t2;
		}
		// UPDATE f and b
		...
		// UPDATE Dk
		Dk = ( 1.0 - mu * mu ) * Dk - f[k + 1] * f[k +
		1] - b[N - k - 1] * b[N - k - 1];
	}
	// ASSIGN COEFFICIENTS
	//coeffs.assign( ++Ak.begin(), Ak.end() );
	//}
	return [Ak[0]*0.1,Ak[Ak.length-1]];
}
...
timeseries.prototype.regression_forecast = function(options) {
	...
	var sample 		= buffer.slice(options.forecast-options.sample, options.forecast);
	console.log("sample",sample);
	var coeffs 		= this.burgCoeff({data: sample});
	console.log("coeffs",coeffs);
	
	for (i=0;i<options.points;i++) {
		console.log("i",i, i+options.forecast, buffer.length);
		buffer[i+options.forecast][1] 	= 0;
		for (j=0;j<coeffs.length;j++) {
			buffer[i+options.forecast][1] += coeffs[j]*buffer[i+options.forecast-1-j][1];
		}
	}
	
	/*
	for (i=options.sample;i<buffer.length;i++) {
		var sample 		= this.data.slice(i-options.sample, i-1);
		...
		for (j=0;j<coeffs.length;j++) {
			buffer[i][1] += coeffs[j]*this.data[i-1-j][1];
		}
	}
	*/
	this.data = buffer;
...
timeseries.prototype.ARMaxEntropy = function(options) {
	...
	for (n=1; n <= options.degree; n++)
	{
		...
		for (j = 0; j < jj; j++)
		{
			t1 = this.data[j + n][1] + pef[j];
			t2 = this.data[j][1] + per[j];
			sn -= 2.0 * t1 * t2;
			sd += (t1 * t1) + (t2 * t2);
		}
		...
		for (j = 0; j < jj; j++)
		{
			per [j] += t1 * pef[j] + t1 * this.data[j + n][1];
			pef [j] = pef[j + 1] + t1 * per[j + 1] + t1 * this.data[j + 1][1];
		}
...
timeseries.prototype.ARLeastSquare = function(options) {
	...
	for (i=options.degree-1;i < length-1;i++) {
		...
		for (j=0;j < options.degree;j++) {
			hj = i - j;
			coefficients[j] += (this.data[hi][1] * this.data[hj][1]);
			for (k=j;k < options.degree;k++) {
				mat[j][k] += (this.data[hj][1] * this.data[i-k][1]);
			}
		}
	}
	for (i=0;i < options.degree;i++) {
...
timeseries.prototype.autoregression = function(options) {
	options = _.extend({
		method:		'ARLeastSquare'	// ARLeastSquare | ARMaxEntropy
	}, options);
...
adapter.sin = function(options) {
	...
	for (i=0;i<options.cycles;i++) {
		for (j=0;j<360;j+=options.quality) {
			output.push([
				new Date(),
				Math.sin(j*Math.PI/180)*100
			]);
		}
	}
	return output;
};
adapter.cos = function(options) {
	options = _.extend({
		cycles:		1
	}, options);
	var i;
