...
indicators.stochastic = function(data,options) {
	...
	for (i=options.period;i<l;i++) {
		var k = (data[i][indicators.C]-indicators.utils.min(data,i,options.period,indicators.C))/(indicators.utils.max(data,i,options.period,indicators.H)-indicators.utils.min(data,i,options.period,indicators.C))*100;
...
// Neural Network Price input (used for price forecasting)
indicators.nnprice = function(data, options) {
	...
	for (i=0;i<l;i++) {
		...
		prices.h.push([data[i][0], data[i][indicators.H]]);
		prices.l.push([data[i][0], data[i][indicators.L]]);
		prices.avg.push([
			data[i][0],
			(data[i][indicators.O]+data[i][indicators.C])/2
		]);
		...
		for (j=1;j<=5;j++) {
			if (j != day) {
				dow[j].push([data[i][0], 0]);
			} else {
				dow[j].push([data[i][0], 1]);
			}
		}
...
// Neural Network Price input (used for price forecasting)
indicators.nnchanges = function(data, options) {
	...
	for (i=options.period+1;i<l;i++) {
		...
		for (j=0;j<options.period;j++) {
			changes[j].push([
				data[i][0],
				(data[i-j][options.price]-data[i-j-1][options.price])/data[i-j-1][options.price]
			]);
		}
...
// Used as the output of the Neural Network training
indicators.outputPrices = function(data, options, transform) {
	...
	var min = indicators.utils.minPrice(data, options.price);
	var max = indicators.utils.maxPrice(data, options.price);
	...
	var l 		= data.length;
	for (i=1;i<l;i++) {
		var index = transform(data[i][0]);
		/*
			Price:	(2*price-(max+min))/(max-min)
			Price:	(price1-price0)/price0
			
			
		*/
		buffer[index] = {
			//"0":	(2*data[i][options.price]-(max+min))/(max-min)
			"0":	(data[i][options.price]-data[i-1][options.price])/data[i-1][options.price]
		};
	}
...
// Used as the output of the Neural Network training
indicators.upOdds = function(data, options, transform) {
	...
	var buffer 	= {};
	var l 		= data.length;
	console.log("ll-Math.max(options.steps)",_.max(options.steps));
	for (i=1;i<l-_.max(options.steps);i++) {
		var index = transform(data[i][0]);
		console.log("index",index);
		buffer[index] = {};
		
		_.each(options.steps, function(step) {
			buffer[index][step]	= false;
			
			for (j=1;j<step;j++) {
				var gain 	= data[i+j][options.price]-data[i][options.price];
				var pct		= gain/data[i][options.price];
				buffer[index][step] |= pct>options.gain;
			}
		});
		
		//buffer[index][j] = data[i+j][options.price]-data[i][options.price] > 0;
	}
...
// Used as the output of the Neural Network training
indicators.futureTrends = function(data, options, transform) {
	...
	for (i=0;i<l;i++) {
		...
		for (j=0;j<options.period;j++) {
			if (options.trend) {
				buffer[index][j] = (data[i+j][options.price] >= data[i][options.price]) * 1;
			} else {
				buffer[index][j] = data[i+j][options.price];
			}
		}
	}
...
indicators.DSPOscillator = function(data,options) {
	...
	//First, we get the Two Pole data
	var TPSM 	= indicators.DSP.TwoPoleSuperSmoother(data,options);
	//var TPSM 	= indicators.DSP.InstaTrend(data,options);
	DSPData		= TPSM[0].data;
	
	var osc		= [];
	
	for (i=1;i<DSPData.length;i++) {
		osc.push([
			DSPData[i][0],
			DSPData[i][1]-DSPData[i-1][1]
		]);
	}
	
	// now we smooth the oscillator's value
	var smoothed	= [];
	var l 			= osc.length;
	for (i=0;i<l;i++) {
		if (i<=options.smoothPeriod) {
			smoothed.push([
				osc[i][0],
				osc[i][1]
			]);
			...
			var tmp = 0;
			var n 	= 0;
			for (j=0;j<options.smoothPeriod;j++) {
				tmp += osc[i-j][1];
				n++;
			}
			smoothed.push([
	...
	// Now we check the direction of the oscillator, for use in a secondary signal
	var direction	= [];
	...
	return [{
		data:	osc,
		name:	"Two Pole Oscillator ("+options.CutoffPeriod+")",
		type:	"spline",
		color:	"000000"
	},{
		data:	smoothed,
		name:	"Smoothed Two Pole Oscillator ("+options.smoothPeriod+")",
		type:	"spline",
		color:	"990000",
		signal:	function(v) {
			if (v > 0) {
		...
		data:	direction,
		name:	"Two Pole Oscillator Direction ("+options.smoothPeriod+")",
		type:	"spline",
		color:	"E15393",
		signal:	function(v) {
			if (v > 0) {
...
indicators.DSP.TwoPoleSuperSmoother = function(data, options) {
	...
	for (i=l;i>=0;i--) {
		...
		if (!buffer[i]) {
			console.log('TwoPoleSuperSmoother error at index '+i);
			break;
		}
	}
